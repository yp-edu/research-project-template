...
4. **Brainstorm**: With the user (or as research-advisor), list 2–5 concrete next-step options (e.g. run literature-review, design a new experiment, write a section, run review-paper). For each option, briefly assess feasibility, impact, and statistical rigor. Flag options that are high-risk or under-specified.
5. **Ideation phase**: If the phase is ideation, apply the ideation checklist before deciding.
6. **Decide**: Work with the user to pick one or more next steps; prioritize and optionally assign a command or subagent (e.g. `/literature-review`, `/design-experiments`, `/write-paper`, `/review-paper`).
7. **Update status**: Use the **project-status** skill to update `docs/PROJECT_STATUS.md`: set Next steps and optionally Phase, Last updated, and Open decisions.
8. **Handoff**: Clearly suggest which command to run next and summarize the plan.
