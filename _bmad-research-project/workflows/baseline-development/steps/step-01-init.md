---
name: 'step-01-init'
description: 'Initialize the baseline development workflow by detecting continuation state and setting up documentation structure'
...
nextStepFile: '{workflow_path}/steps/step-02-sota-understanding.md'
...
To initialize the baseline development workflow by detecting continuation state, setting up the documentation folder structure, and preparing for the first collaborative session.
...
- Look for file at `{workflow_path}/.baseline-development-state.md`
...
- Ask user: "I found an existing baseline development workflow from [date]. Would you like to:
  1. Start a new baseline development session
...
- Create `{project-root}/docs/baseline-development/` if it doesn't exist
- Create `{project-root}/docs/baseline-development/literature/` subfolder
- Create `{project-root}/docs/baseline-development/experiments/` subfolder
- Create README.md files in each folder as placeholders

#### B. Create Initial Sidecar File

Create the sidecar file at `{workflow_path}/.baseline-development-state.md`
...
"Welcome to the Baseline Development workflow! 🔬

This workflow will help you:
- Understand state-of-the-art (SOTA) methods through literature review
- Implement baseline experiments with reproducible code
- Execute experiments and analyze results
- Write initial paper sections (methods, results, related work)

I've set up the documentation structure in `docs/baseline-development/`. Let's begin by understanding the SOTA landscape for your research domain."

### 5. Present MENU OPTIONS

Display: **Proceeding to SOTA Understanding...**
...
- After setup completion, immediately load, read entire file, then execute `{nextStepFile}` to begin SOTA Understanding
...
ONLY WHEN initialization setup is complete and sidecar file is created (OR continuation is properly routed), will you then immediately load, read entire file, then execute `{nextStepFile}` to begin SOTA Understanding.
