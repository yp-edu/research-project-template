...
- Presentations are shareable project updates, meeting decks, talk drafts, and slide sources derived from the project memory.
- Prefer markdown slide sources for new decks so they can be reviewed, diffed, and regenerated.
- Name presentation sources `YYYY-MM-DD-short-title.md` using the intended presentation or update date.
- Separate what the audience sees from what the presenter says: keep visible slide text sparse, and put fuller explanation, caveats, and transitions in presenter notes when the slide format supports them.
- Prefer visual structure over dense prose: include figures, screenshots, diagrams, schemas, tables, or clearly marked placeholders for assets that should be created later.
