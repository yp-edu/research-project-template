...
- Initial setup establishes the project slug, Python package name, paper title, documentation identity, and any cluster paths in `Justfile`.
...
- `results/` is the only location for generated research artifacts, including raw logs, outputs, checkpoints, rendered presentations, and bulky run data.
...
- Every top-level folder under `docs/` has its own `AGENTS.md` with local documentation rules.
...
- Run scripts with `uv run`, for example `uv run -m scripts.<experiment> --config-name <config>`.
- Prefer reusable `just` recipes for common commands: `just checks`, `just tests`, `just run <experiment> <config>`, and `just launch-all --dry-run`.
